...
def wrap_function(fcn,
                  argv=None,
                  optional_arg_types={},
                  extra_args=[],
                  override_arg_defaults={},
                  description=None):
    """
    Construct a command line argument parser for a function, parse the command
    line arguments, and then run the function.
    ...
    override_arg_defaults : dict
        override defaults for arguments in function with those in the dictionary.
        keys are arguments, values are new defaults. This will not edit the 
        docstring default descriptions. 
    ...
    for p in param:
        ...
        if p in override_arg_defaults:
            param_default = override_arg_defaults[p]
        else:
            param_default = param[p].default

        # If no default specified, make required and move on.
        if param_default is param[p].empty:
        ...
        # Otherwise, just grab the type
            ...
            arg_type = type(param_default)

        # bool
        kwargs = {}
        if arg_type is bool:
            if param_default is True:
        ...
        # non-str iterable. either read from a file or take + args in a row
            ...
            kwargs["value_type"] = type(param_default[0])
            kwargs["nargs"] = "+"
