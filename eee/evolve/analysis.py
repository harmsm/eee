...
def get_genotype_frequencies(results,cutoff=50):
    """
    ...
    results : EvolutionResults
        results returned by a Wright-Fisher simulation
    ...
    for g in tqdm(results.generations):
    ...
    for k in keys_to_keep:
        out_dict[k] = np.zeros(len(results.generations),dtype=float)
        for i in range(len(results.generations)):
            try:
                out_dict[k][i] = results.generations[i][k]/population_size
            except KeyError:
                pass
