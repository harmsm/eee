...
from eee.util.write_pdb import write_pdb
from eee.util.load_structure import _load_structure_pdb
...
def test_write_pdb(test_pdbs,tmpdir):
    ...
    df = _load_structure_pdb(test_pdbs["1stn.pdb"])
    write_pdb(df,"test.pdb")

    # Load pdb and write again. This should now be identical
    new_df = _load_structure_pdb("test.pdb")
    write_pdb(new_df,"test2.pdb")
    new_df2 = _load_structure_pdb("test2.pdb")
